...
import { Map, Bus, Clock, Users, Ticket, HardHat, IndianRupee, CheckCircle, RefreshCw, Plus, Pencil, Trash2 } from 'lucide-react';
import api from '../api/axios';
import { useAuth } from '../context/AuthContext';

const STAT_CONFIG = [
  { key: 'routes',         icon: <Map size={18}/>,          label: 'Routes',          color: '#2563eb', bg: '#eff6ff', link: '/routes' },
  { key: 'vehicles',       icon: <Bus size={18}/>,          label: 'Vehicles',        color: '#059669', bg: '#ecfdf5', link: '/vehicles' },
  { key: 'schedules',      icon: <Clock size={18}/>,        label: 'Schedules',       color: '#d97706', bg: '#fffbeb', link: '/schedules' },
  { key: 'passengers',     icon: <Users size={18}/>,        label: 'Passengers',      color: '#7c3aed', bg: '#f5f3ff', link: '/passengers' },
  { key: 'bookings',       icon: <Ticket size={18}/>,       label: 'Bookings',        color: '#0284c7', bg: '#f0f9ff', link: '/bookings' },
  { key: 'staff',          icon: <HardHat size={18}/>,      label: 'Staff',           color: '#dc2626', bg: '#fef2f2', link: '/staff' },
  { key: 'totalRevenue',   icon: <IndianRupee size={18}/>,  label: 'Revenue (₹)',     color: '#059669', bg: '#ecfdf5', fmt: v => '₹'+Number(v).toLocaleString('en-IN') },
  { key: 'activeVehicles', icon: <CheckCircle size={18}/>,  label: 'Active Vehicles', color: '#059669', bg: '#ecfdf5' },
];

const ACTION_ICONS = {
  INSERT: <Plus size={16} color="var(--green)"/>,
  UPDATE: <Pencil size={16} color="var(--amber)"/>,
  DELETE: <Trash2 size={16} color="var(--red)"/>,
};
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill,minmax(180px,1fr))', gap: '10px' }}>
        {STAT_CONFIG.map(s => (
          <div key={s.key} onClick={() => s.link && navigate(s.link)}
            ...
            <div style={{ width: '38px', height: '38px', borderRadius: '9px', background: s.bg, color: s.color, display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>{s.icon}</div>
            <div>
          ...
          <div>
            {audit.map(a => (
              <div key={a.log_id} style={{ display: 'flex', alignItems: 'center', gap: '10px', padding: '10px 16px', borderBottom: '1px solid var(--border)' }}>
                <span style={{ flexShrink: 0 }}>{ACTION_ICONS[a.action]}</span>
